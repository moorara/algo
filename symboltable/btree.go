...
func NewBTree[K, V any](M int, cmpKey CompareFunc[K], eqVal EqualFunc[V]) OrderedSymbolTable[K, V] {
	if M < 2 || M%2 != 0 {
...
// Equals determines whether or not two B-trees have the same key-value pairs.
func (t *btree[K, V]) Equals(rhs SymbolTable[K, V]) bool {
	t2, ok := rhs.(*btree[K, V])
	if !ok {
		return false
	}

	return t._traverse(t.root, t.height, Ascending, func(n *btreeNode[K, V], h int) bool { // t ⊂ t2
		if h == 0 {
			for i := 0; i < n.m; i++ {
		...
		return true
	}) && t2._traverse(t2.root, t2.height, Ascending, func(n *btreeNode[K, V], h int) bool { // t2 ⊂ t
		if h == 0 {
			for i := 0; i < n.m; i++ {
...
func (t *btree[K, V]) AnyMatch(p Predicate2[K, V]) bool {
	return !t._traverse(t.root, t.height, Ascending, func(n *btreeNode[K, V], h int) bool {
		if h == 0 {
			for i := 0; i < n.m; i++ {
...
func (t *btree[K, V]) AllMatch(p Predicate2[K, V]) bool {
	return t._traverse(t.root, t.height, Ascending, func(n *btreeNode[K, V], h int) bool {
		if h == 0 {
			for i := 0; i < n.m; i++ {
				return p(n.children[i].key, n.children[i].val)
...
func (t *btree[K, V]) SelectMatch(p Predicate2[K, V]) Collection2[K, V] {
...
func (t *btree[K, V]) Traverse(order TraverseOrder, visit VisitFunc2[K, V]) {
...
func (t *btree[K, V]) _traverse(n *btreeNode[K, V], h int, order TraverseOrder, visit func(*btreeNode[K, V], int) bool) bool {
	if n == nil {
		return true
	}
...
// Graphviz generates and returns a string representation of the B-tree in DOT format.
