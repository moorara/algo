...
// An indexed maximum heap (priority queue) associates an index to each key-value pair.
...
func NewIndexMaxHeap[K, V any](cap int, cmpKey generic.CompareFunc[K], eqVal generic.EqualFunc[V]) IndexHeap[K, V] {
	pos := make([]int, cap)
	for i := range pos {
		pos[i] = -1
	}
