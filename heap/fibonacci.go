...
func (h *fibonacci[K, V]) verifyTree(n *fibonacciNode[K, V], maxD int) bool {
	...
	for i, stop, child := 1, n.child, n.child; child != nil; i++ {
		...
		// A node of order k has children with orders k-1, k-2, ..., 0 from left to right.
		if child.degree != n.degree-i {
			return false
		}
...
// insert adds a new node to a circular linked list.
//
...
// cut removes a node from a circular linked list.
//
// This method is defined on the fibonacci struct to prevent name clashes
// with other similar implementations in this package.
...
// meld merges two circular linked lists.
//
// This method is defined on the fibonacci struct to prevent name clashes
// with other similar implementations in this package.
...
func (h *fibonacci[K, V]) consolidate() {
	...
	for stop, curr := h.ext, h.ext; ; {
		...
		for y := roots[x.degree]; y != nil && y != x; y = roots[x.degree] {
			 ...
			 * It needs to be updated after cutting a node in case the cut node was h.ext itself.
			 * The actual extremum node will be identified and set later.
			 */
	...
	// Identify and set the actual extremum node.
	for _, r := range roots {
		if r != nil {
...
func (h *fibonacci[K, V]) Delete() (K, V, bool) {
	...
	// Remove the extremum (minimum or maximum) node from the root list.
	// h.ext serves as an entry point to the circular root list too.
	// It needs to be updated after cutting a node in case the cut node was h.ext itself.
	// The actual extremum node will be identified and set later.
	ext := h.ext
	...
	// Merge the children of deleted node with the root list.
	if ext.child != nil {
		h.ext = h.meld(h.ext, ext.child) // The actual extremum node will be identified and set later.
		ext.child = nil
	}
