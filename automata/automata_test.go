...
func TestCmpStates(t *testing.T) {
	tests := []struct {
		name     string
		a, b     States
		expected int
...
func TestHashStates(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expected, HashStates(tc.s))
		})
	}
}
...
func TestEqSymbols(t *testing.T) {
	tests := []struct {
		name     string
		a, b     Symbols
		expected bool
...
func TestCmpSymbols(t *testing.T) {
	tests := []struct {
		name     string
		a, b     Symbols
		expected int
...
func TestHashSymbols(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Ascending",
			s:        NewSymbols('a', 'b', 'c', 'd'),
			expected: 0x146340012207fa8,
		},
		{
			name:     "Descending",
			s:        NewSymbols('d', 'c', 'b', 'a'),
			expected: 0x146340012207fa8,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.expected, HashSymbols(tc.s))
		})
	}
}

func TestUnionStates(t *testing.T) {
	tests := []struct {
		name          string
		a, b          States
