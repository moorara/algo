...
func TestBuildParsingTable(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedErrorStrings: []string{
				`17 errors occurred:`,
				`multiple productions at M[decls, "@left"]:`,
				`decls → decls decl`,
				`decls → ε`,
				`multiple productions at M[decls, "@none"]:`,
				`decls → decls decl`,
				`decls → ε`,
				`multiple productions at M[decls, "@right"]:`,
				`decls → decls decl`,
				`decls → ε`,
				...
				`token → "TOKEN" "=" "REGEX"`,
				`token → "TOKEN" "=" "STRING"`,
				`multiple productions at M[handles, "IDENT"]:`,
				`handles → handles rule`,
				`handles → handles term`,
				`handles → rule`,
				`multiple productions at M[handles, "STRING"]:`,
				`handles → handles rule`,
				`handles → handles term`,
				`handles → term`,
				`multiple productions at M[handles, "TOKEN"]:`,
				`handles → handles rule`,
				`handles → handles term`,
				`handles → term`,
